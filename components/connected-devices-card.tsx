...
export function ConnectedDevicesCard({
  ...
  const wifiCount = allDevices.length;
  const lanCount = upLans.length;
  /** Headline total: wireless clients + active wired links (ports), aligned with the three breakdown tiles. */
  const totalConnected = wifiCount + lanCount;
  const animatedTotal = useCountUp(totalConnected);
  ...
  return (
    ...
    >
      <div
        className={`pointer-events-none absolute right-0 top-0 h-28 w-28 rounded-full ${orbClass}`}
      />
      ...
      <div className="mt-5">
        <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
          ...
          <article
            className={`rounded-[13px] border p-3.5 shadow-sm transition-all duration-200 hover:-translate-y-0.5 hover:shadow-md col-span-2 md:col-span-1 ${
              lanCount > 0
                    ...
                    >
                      ...
                      <div className="mt-2 flex items-center gap-2 text-xs">
                        {(() => {
                          ...
                          return (
                            <span
                              className={`rounded-full px-2 py-0.5 font-medium ${signalMeta.tone}`}
                            >
